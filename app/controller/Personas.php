...
class Personas
{
    ...
    public static function obtener_datos_elemento(){
        $persona = new TablaPersona();
        //return $persona->consulta()->limit(6)->all();
        return $persona->consulta()->where('nombre', 'ximena')->all();
    ...
    public static function insertar_datos(){
        $persona = new TablaPersona();
        return $persona->insercion(['nombre'=>'yesicca', 'apellido'=>'Meza', 'email'=>'yess@email.com',]);
    }
    public static function actualizar_datos(){
        $persona = new TablaPersona();
        return $persona->actualizar(['nombre'=>'Maria', 'apellido'=>'jimenez', 'email', 'mari@email.com'])->where('id_persona', '3')->get();
        //where('id_persona', '3')
    }
